...
@app.post('/volumeDownPi')
async def volumeDownPi():
    global __volumePi__
    global __volumePiMute__
    handleVolumeDownPi()
    return JSONResponse({
...
@app.post('/volumeMutePi')
async def volumeMutePi():
    global __volumePi__
    global __volumePiMute__
    handleMutePi()
    return JSONResponse({
...
async def getFileList(request: Request):
    ...
    #print(f"valid body: {data}")
    genFileList_sh(json.loads(data)["style"])
    #genFileList_sh(style)
    #style=int(body["style"])
    #genFileList_sh(style)
    return JSONResponse({
        "fileList" : __fileList__,
        "musicPiPlaying" : __musicPiPlaying__,
...
async def downPodcastFile(request: Request):
